...
from soft.saab.tasks.forms import TasksListForm
from soft.saab.tasks.model import TasksList


@app.route('/SAAB/tasks_list', methods=['GET', 'POST'])
@login_required
def tasks_list():
    req_tasks_list = TasksList.query.all()
    return render_template(
        'saab/tasks_list/tasks_list.html',
        tasks=req_tasks_list
    )

@app.route('/SAAB/add_tasks_list', methods=['GET', 'POST'])
@login_required
def add_tasks_list():
    form = TasksListForm()
    ...
    return render_template(
        'saab/tasks_list/form_tasks_list.html',
        title="Add task",
        form=form
    )
